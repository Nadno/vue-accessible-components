...
export type KeyboardArrowFocusOrientations = 'horizontal' | 'vertical';
...
const HORIZONTA_KEYS = ['ArrowRight', 'ArrowLeft'],
...
export const useKeyboardArrowFocus = ({
  ...
  const isVerticalHandler = direction === 'vertical',
    handlerKeys = isVerticalHandler ? VERTICAL_KEYS : HORIZONTA_KEYS;
  ...
  const handleFocusByKeyboard = (e: KeyboardEvent) => {
    const key = e.key,
      isArrowFocus = handlerKeys.includes(key),
